...
LD29.Sprite3DRenderer = function(gl) {
  ...
  this.program = new LD29.Program(
    ...
    ["uniform mat4 matrix;",
     "uniform highp vec3 rayOrigin;",
     "attribute vec3 pos;",
     "varying lowp vec3 nearIntersection;",
    ...
    ["const int MAX_ITERATIONS = 22;",
     ...
     "uniform highp vec3 lowBound;",
     "uniform highp vec3 highBound;",
     "uniform highp vec3 rayOrigin;",
     "uniform sampler2D voxelSurfaceMap;",
     "uniform sampler2D voxelColorMap;",
     "varying highp vec3 nearIntersection;",
     "varying highp vec3 rayVector;",
     "",
     "void main() {",
     "  highp vec3 rayDirection = normalize(rayVector);",
     "  highp vec3 rayPosition = nearIntersection;",
     "  highp float rayDistance = 0.0;",
     "  highp vec3 signRayDirection = sign(rayDirection);",
     ...
     "    highp vec2 textureCoord = (vec4(rayPosition, 1) * textureMatrix).xy;",
     "    highp vec3 voxelPosition = floor(rayPosition - rayEntryNormal * 0.5) + 0.5;",
     "    highp vec3 rayEntryOffset = voxelPosition - rayPosition;",
     "    highp vec4 surface = texture2D(voxelSurfaceMap, textureCoord);",
     "    highp vec3 surfaceNormal = surface.xyz;",
     "    highp float surfaceDistance = surface.a - 0.5;",
     ...
     "    highp float rayEntryDistance = dot(rayEntryOffset, surfaceNormal);",
     "    highp float rayExitDistance = dot(rayExitOffset, surfaceNormal);",
     "    if ((rayEntryDistance > surfaceDistance) ||",
     "        (rayExitDistance > surfaceDistance)) {",
     "      highp vec4 color = texture2D(voxelColorMap, textureCoord);",
     "      highp vec3 intersectionNormal;",
     "      if (rayEntryDistance > surfaceDistance) {",
     "        intersectionNormal = rayEntryNormal;",
     "      } else {",
     "        intersectionNormal = surfaceNormal;",
     "      }",
     "      gl_FragColor = color * dot(rayDirection, intersectionNormal);",
     "      return;",
     "    }",
     "    rayEntryNormal = step(1.0, abs(rayPosition - voxelPosition)) * minusSignRayDirection;",
     "    rayDistance += rayDistanceDelta;",
     "    if (rayDistance >= farDistance) {",
     "      discard;",
     "    }",
     "  }",
     "}"]);
