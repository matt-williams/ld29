...
//     "}",
     ...
     "  return voxelPosition + (sign(rayDirection) * 0.5 + 0.5) / 8.0;",
     "}",
...
//     "  highp vec3 rayDirection = normalize(nearPosition - rayOrigin);",
     ...
     "  lowp vec3 faceNormal = step(0.5, abs(nearPosition)) * signRayDirection;",
     "  lowp vec3 voxelPosition = clamp(floor(nearPosition * 8.0) / 8.0, VOXEL_POSITION_MIN, VOXEL_POSITION_MAX);",
     "  lowp vec3 bound = voxelPosition + (signRayDirection * 0.5 + 0.5) / 8.0;",
...
*/
     ...
     "    lowp vec3 voxelStep = faceNormal / 8.0;",
     "    voxelPosition += voxelStep;", 
     "    bound += voxelStep;",
