...
LD29.Sprite3DRenderer = function(gl) {
  ...
  this.program = new LD29.Program(
    ...
    ["const int MAX_ITERATIONS = 22;",
     ...
     "  highp float surfaceDistance;",
     "  highp float rayEntryDistance;",
     "  highp float rayExitDistance;",
     "  for (int iteration = 0; iteration < MAX_ITERATIONS; ++iteration) {",
     "    textureCoord = (vec4(rayPosition, 1) * textureMatrix).xy;",
     ...
     "    }",
     "    gl_FragColor = vec4(color.rgb * (max(dot(rayDirection, intersectionNormal), 0.0) * 0.5 + 0.5), 1.0);",
     "  } else {",
     "    discard;",
     "  }",
     "}"]);
